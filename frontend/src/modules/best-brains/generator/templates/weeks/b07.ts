...
export const buildB07 = makeWeekBuilder({
  ...
  explanation: {
    ...
    whyBeforeHow:
      'A whole lot of something can be split into two parts, and that is a fact about the amounts rather than about the way they are written. So if you know the whole and you know one part, the other part is already decided — there is nothing left to choose. That is why the missing part on the balance is the picture this whole week is built on: a level balance tells you the two sides come to the same amount, and if one side is partly hidden the hidden bit is pinned to whatever the visible side is short of. It cannot be anything else. What a child needs from that is a way IN, and the way in is forwards. Stand on the part you can see, count on until you arrive at the whole, and count your steps as you go: the number of steps is the missing part. Going forwards matters at this age for two reasons. It uses the counting a child already trusts, rather than the taking away they are still building. And it makes the size of the answer obvious — you can feel whether you are nearly there — which is what stops the two wrong answers this week exists to remove. The first is answering with the whole, because the whole is the biggest number on the page and looks important. The second is adding the two printed numbers, which a child who has spent every previous week adding will do without noticing. Both are ruled out before any arithmetic by one question: a part cannot be bigger than the whole it came out of. Once the count is secure it can be written down, and a box is just the place a number sits before you know it. The box may stand first or second in the sentence, because the two parts of a whole do not take turns.',
    script: [
      {
