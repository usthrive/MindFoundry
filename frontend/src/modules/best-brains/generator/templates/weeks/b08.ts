...
export const buildB08 = makeWeekBuilder({
  ...
  explanation: {
    ...
    whyBeforeHow:
      'Two parts and the whole they make are not three separate numbers. They do not simply happen to get on well. They are one relationship. That is why the part-part-whole triangle is the picture this week is built on. The whole sits in the top corner and the two parts sit below it. The three of them cannot be rearranged without breaking the relationship. Because there is only one relationship, there is really only one fact. The four cards a child writes are four ways of saying it out loud. They are not four things to remember. Two consequences run through every page here. The first is about adding. Six and nine make fifteen, and nine and six make fifteen too. That is because the two parts sit side by side and neither is first. A child who has to rebuild the second sum from scratch has missed something. They have not yet seen that it is the same sum. The second is about taking away, and it is the one that pays. A take-away card can only ever open with the whole. The whole is the only one of the three that has parts inside to remove. So fifteen take away six is not a journey backwards at all. It is a question about a partner. That partner is already written on the adding card. That is what makes this week worth a week. A child who owns one family owns every take-away inside twenty that it covers. They never have to count back once. One habit has to be guarded against, and it is real and common. A child who has met the three numbers and not the relationship writes anything. They write any sentence those numbers will fit into. That can include five take away four makes nine. The repair is not arithmetic. It is asking which of the three numbers is the whole. That question comes before a single sign is written down.',
    script: [
      {
