...
export const buildB04 = makeWeekBuilder({
  ...
  explanation: {
    ...
    whyBeforeHow:
      'A hop is not something you add to a number. It is a move you make from one square to the next. That is why number-line hops are the model this whole week is built on. Two things follow from it, and they are the two things children get wrong. First: the square you are standing on is not a hop. A hop is the gap between two squares. So the very first hop lands on the square NEXT to you. A child may say their own number as hop one. That child arrives one square out, every single time. And they arrive there confidently. Second: nothing about the numbers themselves says which way to travel. From square fourteen, three hops reach seventeen or they reach eleven. Both are correct arithmetic, and only the words settle it. That is why this week rotates how the story says it. On, forward, up the track, back, down the track. A child who has only ever met the word "back" learns to spot a word. They do not learn to read a story. Comparing numbers was settled last week. It deliberately left the direction question alone. So this is where that question is asked. Once both halves are secure, two hops can be strung together. Make the first move and stop on the square it reaches. Then read the story again for the second hop. The hardest version is not a big number, it is a small turn.',
    script: [
      {
