...
export const buildB10 = makeWeekBuilder({
  ...
  explanation: {
    ...
    whyBeforeHow:
      'Ask a six-year-old what four and three make and they will tell you at once. Ask them what forty and thirty make and many will stop. Some will even write 43. That is not a gap in their arithmetic. It is a question about what the numeral 30 stands for. It is worth answering properly before any sum is attempted. The answer is the ten-block. A ten-block is ten ones fastened together so that they travel as one object. Once ten ones have become one thing, they can be counted like anything else. Four ten-blocks and three ten-blocks are seven ten-blocks. That is because four and three do the same to anything. Seven ten-blocks is seventy. So the sum a child already owns does all the work. The only new step is agreeing what is being counted. That is why these pages say "whole tens" out loud instead of showing written sums. A child who reads 40 + 30 digit by digit sees a four and a three. Nothing stops them writing 43. Now hear it as "four whole tens and three whole tens". That child has already been told what to count. The second half of the week follows from the first. If only tens were added, then only tens can have changed. So the ones digit of the answer is the ones digit you started with. Thirty-four with twenty put on is fifty-four. The four is untouched, because nothing arrived that had any ones to give. That is not a trick for getting answers. It is a check any child can run without help. It catches the very mistake this week exists to prevent. A number whose last digit has moved was never really adding tens at all.',
    script: [
      {
