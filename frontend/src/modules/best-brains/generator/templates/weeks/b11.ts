...
export const buildB11 = makeWeekBuilder({
  ...
  explanation: {
    ...
    whyBeforeHow:
      'A child who can work out eight and six can be stopped cold by thirty-eight and six, and it is worth being clear about what has actually changed, because it is not the adding. The arriving six has nowhere to go except the ones column, so the only question is whether the ones column can hold it. That is why these pages say ones meet ones before they say anything else: the tens standing in front are not part of the sum at all, they are simply already there. Now look at the ones. Thirty-eight has eight loose ones and room for two more before the row is full. Six arrive. Two of them finish the ten and four are left over, so the count has one more whole ten than it had, and four loose ones on top of it. Forty-four. Nothing was carried, borrowed or written in a column; a ten was built out of loose ones and put where whole tens live. And that is the whole difference between the two shapes this week draws. When the arrivals fit in the room that was left, the front digit never stirs and the answer is finished in the ones. When they do not fit, a new ten begins and the front digit climbs by exactly one — never by two, because nine loose ones and nine more cannot build two tens between them. So a child can check any answer here before they even trust it: count the room, decide whether a new ten begins, and see whether the answer they wrote agrees.',
    script: [
      {
