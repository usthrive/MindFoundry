 ...
 *    is MOVED to where it can be shown honestly and is carried in three places
 *    at once: it is a live option on every `seeOrHide` draw (the "other part",
 *    offered 100% of the time and keyed only when the question names it), it is
 *    a live option in the puppet item itself, and it heads the mistakeBank with
 *    its own distractor rationale. What the puppet DOES is the derivable
 ...
 *
 * 3. **THE ANSWER SPACE IS FOUR VALUES WIDE, so the mastery slots were budgeted
 *    for width before the items were written.** A partner of five is 1, 2, 3 or
 *    4 — never 0 (an option no child looking at a covered frame could pick, the
 *    shape `howManyChoice` was fixed for) and never 5 (the frame's own capacity).
 *    That is exactly the shape that manufactures a dead option, so:
 *
 *    - **Four of the six mastery slots carry NO options at all.** Slots 01, 02,
 *      04 and 05 are small free-entry numerics — the band's other sanctioned
 *      answer mode — so there is no rank to sit at and no option to strike out.
 *      This is not dodging the gate: it is the honest reading of a four-cell
 *      answer space, and it moves the certification onto producing the partner
 *      rather than choosing it.
 *    - **The two choice slots were built to be honest and then measured.**
 *      `seeOrHide` keys the shown part half the time and the hidden part half the
 *      time, so every value 1–4 is keyed and the "other part" option is a live
 *      answer rather than a permanent decoy. Measured over 400 packs across its
 *      four slots: every value keyed on 21.8–29.0% of draws, the key at rank 1 of
 *      3 on 30.3–34.5%, rank 2 on 47.5–53.5%, rank 3 on 15.5–18.0%, and the one
 *      never-keyed option ("5", the whole frame) offered on 29.5–33.5% — a third
 *      of the pages, well under the 50% at which `bb-answer-entropy-test` calls
 *      an option dead. `puppetCountsOn` keys every value on 22.0–29.0%, sits at
 *      rank 1 on 47.8–53.5% and rank 2 on 46.5–52.3%, and offers "5" on
 *      34.5–39.3%. Both mastery pairs are inside those ranges; nothing anywhere
 *      in the week reaches 54% at a rank, against a 95% gate.
 ...
 *      sometimes be correct, which stops being error analysis. And on `seeOrHide`
 *      the two blind habits — "always say the part you can see" and "always say
 *      the partner" — are complementary, so exactly one is right on every draw
 *      and balancing them is the only honest defence: measured 51.3% and 48.8%
 *      over 1,600 draws, against a 33.3% chance floor. Note which one that
 *      punishes. "Always say the partner" requires the bond to have been made,
 *      so it diagnoses a child who is not listening; "always say the part you can
 *      see" requires no bond at all, and it is the one held to a coin. Reported,
 *      not hidden.
 *
 ...
 *
 * 7. **`allWaysToMake`'s completeness claim is NOT audited by any gate, so this
 *    week re-derives it.** `a_all_ways_v1` registers an `answerFor` that rebuilds
 *    every ordered pair, but QG-5's arithmetic re-check only runs for four
 *    validations and `'set'` is not one of them (`validator.ts:461`) — measured,
 *    a pack with a pair deleted from that answer still validates clean. The pin
 *    is registered, green and load-bearing on nothing. Reported with a proposed
 *    fix; locally, `withCheckedWays` enumerates the pairs a second time and
 *    throws on disagreement, so this week's Day-5 claim is checked even though
 *    the corpus's is not.
 *
 ...
 * highest in turn: measured over 400 packs and this generator's four slots, rank
 * 1 of 3 on 30.3–34.5% of draws, rank 2 on 47.5–53.5%, rank 3 on 15.5–18.0%. The
 * whole is held to one bucket in three, so it is offered on 29.5–33.5% of pages
 ...
 * 47.8–53.5% smallest and 46.5–52.3% middle over 400 packs and this generator's
 * three slots, against the 95% at which `bb-answer-entropy-test` calls a rank
 * constant.
 *
 ...
 *                       34.5–39.3% of pages rather than three quarters of them
 *
 ...
 *
 * `a_all_ways_v1` registers an `answerFor` that rebuilds the whole list, and the
 * library says so; what neither says is that NOTHING EVER CALLS IT. QG-5's
 * arithmetic re-check is gated on `['exact-numeric', 'equivalent-numeric',
 * 'equivalent-fraction', 'ordered-list']` (`validator.ts:461`) and this item
 * validates as `'set'`, so the pin is registered, green and dead. Measured
 * directly: dropping a pair from a generated pack's answer leaves `validatePack`
 * reporting 0 violations at contract v2. It is not local to this week —
 * `money.ts` and eight week files ship `'set'` answers behind the same hole.
 *
 * Reported for the orchestrator with a proposed fix. Meanwhile the guarantee is
 * rebuilt here the way a11 rebuilt its pattern table: a SECOND, independent
 * enumeration of the ordered pairs, compared with what the family drafted. Two
 * implementations that must agree is a real re-derivation; if either drifts,
 * every seed throws at once instead of one page quietly shipping short.
 */
...
const seeOrHideIt = seeOrHide();
const puppetMixUp = puppetCountsOn();
const storyCard = cardStory();
const storyFill = fillStory();
...
export const buildA12 = makeWeekBuilder({
  ...
  isomorphNotes:
    'Pairs by index; same generator and difficulty per slot, fresh cells off a separate stream. 01: how many hide under the card, typed as a number. 02: the same bond written as a sentence with a box for the unknown. 03: one covered frame asked either way - what you can see, or what is hiding - so the part not asked for is a live option rather than a permanent decoy. 04: a story whose whole is stated and whose second part is under a card. 05: a story with no number stated at all, where the missing part is still an open gap. 06: the puppet who whispers on but counts the box he started from. Four of the six slots take a typed number rather than a tap, because a partner of five has four possible values and a three-option page over four values manufactures an option that is never right; the two that do offer options were measured for it. Every bond cell is spent exactly once across the daily pages and the two forms, so no slot is left holding the leftovers.',
  mistakeBank: [
    {
      ...
      distractorRationale:
        'Offer the other part of five on every partner choice item. It is offered on every single draw and is keyed only when the question actually names that part, so it can never be struck out for free.',
    ...
    },
    {
      ...
      distractorRationale:
        'Offer the whole beside the parts on a third of the partner choice items. It is never the answer - a part of five is never five - so its offer rate is held to one bucket in three rather than half the pages.',
    ...
    },
    {
      ...
      distractorRationale:
        'Offer one more than the true partner. It is the number the puppet says out loud, and it is recomputed from the frame rather than authored.',
    ...
    },
    {
      ...
      distractorRationale:
        'Ask both questions over one drawn frame and offer the other part either way, so answering by habit is wrong exactly half the time and answering by listening is always right.',
