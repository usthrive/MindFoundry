...
export const buildB09 = makeWeekBuilder({
  ...
  explanation: {
    ...
    whyBeforeHow:
      'A six-year-old who cannot start a word problem is almost never stuck on the arithmetic. Ask them what eight and five make and they will tell you; put the same sum inside four sentences about a basket and they will look for a signal — a word they have been told means add, or simply the two numbers with a plus between them. That is a guess, and it works often enough to survive and rarely enough to hurt. The repair is the story frame, and it is worth teaching as a shape because a shape can be checked when a hunch cannot: a story about a lot of things has three parts, which are what there was, what happened, and what there is now. Two of the three are always printed and the question always wants the third. So the first move on any of these pages is not arithmetic at all. It is reading twice and asking which of the three is missing. Once that is settled the arithmetic settles itself, because there is only ever one sensible thing to do with the two numbers that are left. That is also why the same three numbers can want different moves on different pages without anything having tricked anybody. A story that joins still needs a count upwards when the part it withholds is the join itself: the basket held six, more went in, and now there are eleven, so finding what went in means walking from six up to eleven. Nothing lied. The verb told the truth about what happened, and it was never the verb job to say which of the three parts was missing. One more thing runs through the week and it is the thing children find hardest. The numbers printed in a story are not always the numbers you use. When a sentence says three more went in than in the morning, it has named an amount without printing it, and a child who reaches for the three has answered a question nobody asked. Building that amount first, before anything is joined or taken, is the habit these pages are here to lay down.',
    script: [
      {
